<template>
  <view class="container">
    <view class="page-header">
      <text class="page-title">查看明细</text>
    </view>
    <AccountForm
...
onMounted(async () => {
  ...
  if (query.id) { isEdit.value = true; editId.value = query.id; await loadAccountDetail(query.id) }
})
...
async function handleSubmit() {
  ...
  try { await request('/account/' + editId.value, 'PUT', data); uni.showToast({ title: '保存成功' }); setTimeout(() => uni.navigateBack(), 1500) }
  catch (err) { console.error('提交失败', err) }
  finally { uni.hideLoading() }
}
function handleDelete() {
  uni.showModal({
    title: '确认删除', content: '确定要删除这条账目吗？',
    success: async (res) => {
      if (res.confirm) {
